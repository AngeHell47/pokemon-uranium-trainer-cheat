...
| Inventaire complet | Snapshot dédupliqué de toutes les poches, catalogue runtime, `pbCanStore?`/`pbStoreItem`/`pbDeleteItem`, ID+quantité figés dans une FIFO | Une quantité refusée par la poche reste inchangée ; plafond d'interface 999 | 0, 1, 99, 100, 999, poche pleine, objet clé, chaque poche |
| Noclip | Lit directement l'état physique de la touche clavier configurable (`Ctrl` par défaut) ou d'un bouton XInput assigné, ou reste actif sans raccourci ; surclasse temporairement `@through` pendant `passable?`, puis restaure l'état original | Les limites externes de carte restent actives ; `Inser` annule la saisie du raccourci, tandis que `Échap`, Retour arrière et Suppr assignent `none` | Murs, événements, portes, bords de carte, état scripted-through |
...
| Vitesse globale | Exécute 1 à 5 ticks RGSS à cadence accélérée, virtualise `Graphics.frame_count`/`frame_rate`, augmente progressivement le rendu avec un plafond absolu de 120 FPS et lit directement une touche clavier ou un bouton XInput configurable | Ne modifie pas l'horloge Windows et limite le coût du rendu ; l'audio conserve sa vitesse/pitch naturels | Fluidité des déplacements améliorée ; dialogues, combats, animations, transitions et cinématiques validés en jeu jusqu'à x5 |
...
| Éditeur Pokémon complet | Snapshot équipe+boîtes, détail ciblé, FIFO par emplacement, nom 11 caractères, IV 0–31, EV 0–255/total 510, PP/PP Up, création et suppression confirmée | Un déplacement natif simultané peut invalider la cible : la commande est alors refusée ; toute édition est sauvegardable | Équipe 1–6, plusieurs boîtes, chaque champ, création équipe/boîte pleine, suppression dernier membre refusée |
