#include "opt_trainer_capture.h"
#include "../rgss_safe_dispatch.h"

#include <stdio.h>

// Uranium emploie deja le chemin de capture "Snag Ball" pour retirer un
// Pokemon d'un dresseur sans terminer le combat. Le hook reutilise ce chemin
// officiel au lieu de remplacer pbThrowPokeBall et ses animations/scripts.
bool g_capture_trainers = false;

static char s_ini[MAX_PATH] = {};
static volatile LONG s_enabled = 0;
static volatile LONG s_pending = 0;
static volatile LONG s_installed = 0;
static volatile LONG s_retry_started = 0;
static char s_ruby[4096] = {};

static void notify_game() { rgss_safe_dispatch_notify(); }

static void build_ruby() {
    const char* enabled = InterlockedExchangeAdd(&s_enabled, 0) ? "true" : "false";
    _snprintf_s(s_ruby, sizeof(s_ruby), _TRUNCATE,
        "installed=0\n"
        "begin\n"
        "  $__uranium_trainer_capture_trainers=%s\n"
        "  if defined?(::PokeBattle_Battle) && ::PokeBattle_Battle.method_defined?(:pbIsSnagBall?)\n"
        "    class ::PokeBattle_Battle\n"
        "      unless instance_methods.collect { |m| m.to_s }.include?(\"__uranium_trainer_original_pbIsSnagBall\")\n"
        "        alias_method :__uranium_trainer_original_pbIsSnagBall, :pbIsSnagBall?\n"
        "      end\n"
        "      def pbIsSnagBall?(ball)\n"
        "        return true if $__uranium_trainer_capture_trainers && @opponent\n"
        "        __uranium_trainer_original_pbIsSnagBall(ball)\n"
        "      end\n"
        "    end\n"
        "    installed=1\n"
        "  end\n"
        "rescue Exception\n"
        "end\n"
        "begin\n"
        "  Win32API.new(\"kernel32\",\"RtlMoveMemory\",[\"l\",\"p\",\"l\"],\"v\").call(%lu,[installed].pack(\"l\"),4)\n"
        "rescue Exception\n"
        "end\n",
        enabled, (unsigned long)(ULONG_PTR)&s_installed);
}

static void __cdecl on_game_thread_tick(void*) {
    if (InterlockedExchange(&s_pending, 0) == 0) return;
    build_ruby();
    if (rgss_safe_eval(s_ruby) != 0) InterlockedExchange(&s_pending, 1);
}

static DWORD WINAPI retry_thread(LPVOID) {
    while (InterlockedExchangeAdd(&s_installed, 0) == 0 &&
           !rgss_safe_dispatch_is_stopping()) {
        InterlockedExchange(&s_pending, 1);
        notify_game();
        Sleep(500);
    }
    InterlockedExchange(&s_retry_started, 0);
    return 0;
}

static void ensure_retry_thread() {
    if (InterlockedExchangeAdd(&s_installed, 0) != 0 ||
        InterlockedCompareExchange(&s_retry_started, 1, 0) != 0) return;
    HANDLE thread = CreateThread(NULL, 0, retry_thread, NULL, 0, NULL);
    if (thread) CloseHandle(thread);
    else InterlockedExchange(&s_retry_started, 0);
}

void opt_trainer_capture_init(const char* ini_path) {
    lstrcpynA(s_ini, ini_path ? ini_path : "", MAX_PATH);
    g_capture_trainers = GetPrivateProfileIntA("Settings", "CaptureTrainers", 0, s_ini) != 0;
    InterlockedExchange(&s_enabled, g_capture_trainers ? 1 : 0);
    InterlockedExchange(&s_pending, 1);
    InterlockedExchange(&s_installed, 0);
}

void opt_trainer_capture_set_hwnd_and_start(HWND) {
    rgss_safe_dispatch_register(on_game_thread_tick, NULL);
    InterlockedExchange(&s_pending, 1);
    notify_game();
    ensure_retry_thread();
}

void opt_trainer_capture_toggle(bool enabled) {
    g_capture_trainers = enabled;
    InterlockedExchange(&s_enabled, enabled ? 1 : 0);
    WritePrivateProfileStringA("Settings", "CaptureTrainers", enabled ? "1" : "0", s_ini);
    InterlockedExchange(&s_pending, 1);
    notify_game();
    ensure_retry_thread();
}
