...
static void read_profile_on_game_thread() {
    ...
    const char* ruby =
        ...
        "  gender=(global ? (global.playerID rescue 0) : (trainer.gender rescue 0)).to_i\n"
        "  base=(defined?($totalPlayTime) ? $totalPlayTime : 0)\n"
        "  base=(base.is_a?(String) ? base[1..-1].to_i : base.to_i)\n"
        "  loaded=(defined?($loadTime) ? $loadTime.to_i : Time.now.to_i)\n"
        "  seconds=base+[Time.now.to_i-loaded,0].max\n"
        "  badges=(trainer.badges rescue trainer.instance_variable_get(:@badges) rescue [])\n"
        "  mask=0\n"
        "  8.times { |i| mask|=(1<<i) if (badges[i] rescue false) }\n"
        "  trainer_id=(trainer.id rescue trainer.instance_variable_get(:@id) rescue 0).to_i\n"
        "  data=name+[gender,seconds,mask,trainer_id].pack('l4')\n"
...
static void write_profile_on_game_thread() {
    ...
    _snprintf_s(code, sizeof(code), _TRUNCATE,
        ...
        "  pbChangePlayer(%d) if defined?(pbChangePlayer)\n"
